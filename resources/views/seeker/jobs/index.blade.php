        ...
        <div class="card bg-base-100 shadow-xl">
            <div class="card-body">
                <form method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-2">
                    ...
                    <input name="skills" class="input input-bordered" value="{{ request('skills') }}" placeholder="Skills: plumbing, tiling">
                    <input name="county" class="input input-bordered" value="{{ request('county') }}" placeholder="County">
                    <div class="sm:col-span-3">
                        <button class="btn">Filter</button>
                        <a href="{{ route('seeker.jobs.index') }}" class="btn btn-ghost ml-2">Reset</a>
                    </div>
                </form>
                ...
                <div class="divide-y">
                    @forelse($jobs as $job)
                        <div class="py-4">
                            <a href="{{ route('seeker.jobs.show', $job) }}" class="link link-primary text-lg font-semibold">{{ $job->title }}</a>
